...
import com.me.viewcount.v2.MyServiceV2;
...
@Component
@Slf4j
public class MemoryViewCounterV3 implements ViewCounter {

    private static final String VALUE_FORMAT = "%s-%s";


    private final ConcurrentLinkedQueue<String> eventQue;
    private final MyServiceV3 viewCounter;

    public MemoryViewCounterV3(MyServiceV3 viewCounter) {
    ...
    public void commit() {
        ...
        eventSet.stream()
                .map(EventDto::create)
                .forEach(eventDto -> viewCounter.tx1(eventDto.getCustomerId(), eventDto.getStoreId()));
    }
