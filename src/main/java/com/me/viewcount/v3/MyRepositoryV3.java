...
public class MyRepositoryV3 {


    private final EntityManager em;


    public boolean acquireNamedLock(String lockName) {
        final int timeout = 10;

        Query query = em.createNativeQuery("SELECT GET_LOCK(:lockName, :timeout)");
    ...
    public void releaseNamedLock(String lockName) {
        Query query = em.createNativeQuery("SELECT RELEASE_LOCK(:lockName)");
        query.setParameter("lockName", lockName);
